...
function GaugeCard({ w, now }: { w: QWindow; now: number }) {
  ...
  return (
    <div className="card">
      <div className="gauge-head">
        <span className="gauge-label">Codex</span>
      ...
      <div className="track">
        <div className={`fill ${st}`} style={{ width: `${width}%` }} />
      </div>
      <div className="reset">
...
export default function Home() {
  ...
  return (
    <main className="app">
      ...
      {snap && snap.codex.available ? (
        <div className="grid">
          {snap.codex.windows.map((w) => (
            <GaugeCard key={w.window} w={w} now={now} />
          ))}
        </div>
      ) : (
        <div className="card">
          <div className="sub">
            {snap
              ? `Codex quota unavailable${snap.codex.degraded ? `: ${snap.codex.degraded}` : ""}.`
              : "Loading…"}
          </div>
        </div>
      )}
